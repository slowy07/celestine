...
class Renderer:
    def __init__(
        ...
        if self.renderer in ["markerpen"]:
            ...
            self.brush_small_vertical = cv2.imread(
                r"../polisher/brush_small_vertical.png", cv2.IMREAD_GRAYSCALE
            )
            self.brush_small_horizontal = cv2.imread(
                r"../polisher/brush_small_horizontal.png", cv2.IMREAD_GRAYSCALE
            )
            self.brush_large_vertical = cv2.imread(
                r"../polisher/brush_large_vertical.png", cv2.IMREAD_GRAYSCALE
            )
            self.brush_large_horizontal = cv2.imread(
                r"../polisher/brush_large_horizontal.png", cv2.IMREAD_GRAYSCALE
            )
            ...
            raise NotImplementedError(
                f"ERROR: invalid  renderer name {self.renderer} chose from [watercolor, markerpen, oilpaintbrush, rectangle]"
            )
    ...
    def random_stroke_params_sampler(self, err_map, image):
        map_h, map_w, c = image.shape
        ...
        if self.renderer in ["markerpen"]:
            ...
            color = image[int(cy * map_h), int(cx * map_w), :].tolist()
    ...
    def _draw_watercolor(self):
        ...
        self.foreground = np.zeros_like(self.canvas, dtype=np.uint8)
        self.stroke_alpha_map = np.zeros_like(self.canvas, dtype=np.uint8)
    ...
    def _draw_oilpaintbrush(self):
        ...
        if not self.train:
            self.foregtound = cv2.dilate(self.foreground, np.ones([2, 2]))
            self.stroke_alpha_map = cv2.erode(self.stroke_alpha_map, np.ones([2, 2]))
        self.foreground = np.array(self.foreground, dtype=np.float32) / 255.0
        self.stroke_alpha_map = (
            np.array(self.stroke_alpha_map, dtype=np.float32) / 255.0
    ...
    def _update_canvas(self):
        return self.foregrond * self.stroke_alpha_map + self.canvas * (
