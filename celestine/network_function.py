...
def get_norm_layer(norm_type="instance"):
    if norm_type == "batch":
        ...
        raise NotImplementedError(
            f"ERROR: normalization layer f{str(norm_type)} is not found"
        )
    return norm_layer
    ...
    def init_func(m):
        ...
        if hasattr(m, "weight") and (
            ...
            if init_type == "normal":
                ...
                init.kaiming_normal_(m.weight.data, gain=init_gain)
            elif init_type == "orthogonal":
                init.orthogonal_(m.weight.data, gain=init_gain)
            else:
                raise NotImplementedError(
                    f"Error: initializatopm method {str(init_type)}"
                )
        ...
        print(f"initialize network with {str(init_type)}")
        net.apply(init_func)
...
def define_G(rdrr, netG, init_type="normal", init_gain=0.02, gpu_ids=[]):
    ...
    if netG == "plain-dcgan":
        ...
        raise NotImplementedError(
            f"ERROR: generator model name {str(netG)} is not recognized"
        )
...
class HmodelNet(nn.Module):
    def __init__(self, rdrr):
        super(HmodelNet, self).__init__()
        self.rdrr = rdrr
        self.out_size = 128
...
class FCNFusion(nn.Module):
    def __init__(self, rdrr):
        super(FCNFusion, self).__init__()
        self.rdrr = rdrr
        self.out_size = 128
...
class FCNFusionLight(nn.Module):
    def __init__(self, rdrr):
        super(FCNFusionLight, self).__init__()
        self.rdrr = rdrr
        self.out_size = 32
...
class UnetGenerator(nn.Module):
    def __init__(
        ...
        net_block = UnetSkipConnectionBlock(
