...
class SinkhornLoss(nn.Module):
    ...
    def forward(self, canvas, gt):
        ...
        canvas_grids = self._mesh_grid(batch_size, h, w)
        gt_grids = torch.clone(canvas_grids)
        i = random.randint(0, 2)
        image_1 = canvas[:, [i], :, :]
        image_2 = gt[:, [i], :, :]
        mass_x = image_1.reshape(batch_size, -1)
        mass_y = image_2.reshape(batch_size, -1)
        if self.normalize:
