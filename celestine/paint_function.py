...
class PainterBase:
    def __init__(self, args):
        ...
        self.net_G = define_G(rdrr=self.rderr, netG=argus.net_G).to(device)
        self.x_ctt = None
        self.x_color = None
        self.x_alpha = None
    ...
    def _load_checkpoint(self):
        if os.path.exists((os.path.join(self.renderer_checkpoint_dir, "last_ckpt.pt"))):
            print("INFO: loading render from pre-trained checkpoint...")
            ...
            print("INFO[WARN]: pre-trained renderer does not exists...")
            exit()
    ...
    def _save_stroke_params(self, v):
        ...
        np.savez(
            filename + "strokes.npz", x_ctt=x_ctt, x_color=x_color, x_alpha=x_alpha
        )
    ...
    def _render(self, v, save_jgps=True, save_video=True):
        v = v[0, :, :]
        if self.args.keep_aspect_ratio:
            if self.input_aspect_ratio < 1:
                out_h = int(self.args.canvas_size * self.input_aspce_ratio)
        ...
        if save_jpgs:
            ...
            plt.imsave(filename + "_input.png", out_img)
    ...
    def _normalize_strokes(self, v):
        ...
        if self.rderr.renderer in ["enumerate", "markerpen"]:
            xs = np.array([0, 4])
            ys = np.array([1, 5])
            rs = np.array([6, 7])
            ...
            raise NotImplementedError(
                f"ERROR: renderer {str(self.rderr.renderer)} is not implemented"
            )
    ...
    def stroke_sampler(self, anchor_id):
        ...
        for i in range(self.m_grid * self.m_grid):
            ...
            self.rderr.random_stroke_params_sampler(
                err_map=this_err_map, image=this_imag
            )
    ...
    def _forward_pass(self):
        ...
        self.G_pred_foreground = morphology.Dilation2d(m=1)(self.G_pred_foreground)
        self.G_pred_alpha = morphology.Erosion2d(m=1)(self.G_pred_alpha)
        ...
        self.G_pred_alpha = torch.reshape(
            self.G_pred_alpha,
            [
                self.m_grid * self.m_grid,
                self.anchor_id + 1,
        ...
        for i in range(self.anchor_id + 1):
            ...
            G_pred_alpha = self.G_pred_alpha[:, i]
            self.G_pred_canvas = (
...
class Painter(PainterBase):
    def __init__(self, args):
        ...
        self.final_rendered_image = None

    def _drawing_step_state(self):
        acc = self._compute_acc().item()
        print(
            f"INFO: iteration step {int(self.step_id)}, G_loss: {self.G_loss.items()} step_psnr: {acc} ",
            f"strokes: {int((self.anchor_id+1)*self.m_grid*self.m_grid)} / {self.max_m_strokes}",
        )
        vis2 = utils.patches_to_image(self.G_final_pred_canvas, self.m_grid).clip(
            min=0, max=1
        )
        if self.args.disable_priview:
            pass
        else:
...
class ProgressivePainter(PainterBase):
    ...
    def _drawing_step_state(self):
        acc = self._compute_acc().item()
        print(
            f"INFO: iteration step {self.step_id}, G_loss: {self.G_loss.item()} step_acc: {float(acc)}, "
            f"grid_scale: {int(self.m_grid)} / {int(self.max_divide)}, strokes:  {int(self.anchor_id) + 1} / {int(self.m_strokes_per_block)}"
        )
        vis2 = utils.patches_to_image(self.G_final_pred_canvas, self.m_grid).clip(
            min=0, max=1
        )
...
class NeuralStyleTransfer(PainterBase):
    def __init__(self, args):
        ...
        if os.path.exists(args.vector_file) is False:
            exit(
                "INFO: vector file does not exists, please check --vector_file, or run demo.py first"
            )
        else:
            npzfile = np.load(args.vector)
        ...
        self.anchor_id = self.x_ctt[1] - 1

        image_ = cv2.imread(args.content_image_path, cv.IMREAD_COLOR)
        ...
        self.style_image = cv2.blur(cv2.resize(self.style_image_, (128, 128)), (2, 2))
        self.style_image = (
    ...
    def _style_transfer_step_states(self):
        acc = self._compute_acc().item()
        print(
            f"INFO: running style transfer... iteration step {int(self.step_id)} "
            f"G_loss: {self.G_loss.item()}, step_psnr: {acc}"
        )
        vis2 = utils.patches_to_image(self.G_final_pred_canvas, self.m_grid).clip(
            min=0, max=1
        )
    ...
    def _save_style_transfer_images(self, final_rendered_image):
        if self.args.keep_aspect_ratio:
            ...
            out_h = self.args.canvas_size
            out_w = self.args.canvas_size
        print("INFO: saving transfer image style ...")
        ...
        out_image = cv2.resize(final_rendered_image, (out_w, out_h), cv2.INTER_AREA)
        plt.imsave(
            filedir
            + "_style_transfer_"
            + self.style_image_path.split("/")[-1][:-4]
